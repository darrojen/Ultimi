...
//         const dateStr = new Date(Date.UTC(selectedYear, month.index, day)).toISOString().split('T')[0]
...
//                     <Box as="h4" className={`text-center mb-2 ${theme === 'custom' ? 'text-gray-900' : theme === 'light' ? 'text-gray-900' : 'text-cyan-300'}`}>
//                       {month.name} {selectedYear}
//                     </Box>
//                     <Box className="grid grid-cols-7 gap-1 text-xs">
//                       {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
//                         <Box as="div" key={day} className={`text-center ${theme === 'custom' ? 'text-gray-900' : theme === 'light' ? 'text-gray-900' : 'text-cyan-300'}`}>
//                           {day}
//                         </Box>
//                       ))}
...
export const ActivityCalendar: React.FC<ActivityCalendarProps> = ({
  ...
  const allCalendarDays = useMemo(() => {
    return months.map((month) => {
      ...
      // Actual days
      for (let day = 1; day <= daysInMonth; day++) {
        const dateStr = new Date(Date.UTC(selectedYear, month.index, day))
          .toISOString()
          .split('T')[0]
        calendarDays.push({ date: dateStr, count: activityData[dateStr] || 0 })
      }
        ...
        <Carousel
          setApi={(api) => {
            if (api) {
              ...
              setTimeout(() => {
                api.scrollTo(currentMonthIndex, true) // smooth transition
