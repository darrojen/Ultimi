              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-2 gap-4">
                {scienceSubjects.map((sub) => (
                  <motion.div
                    key={sub}
                    whileHover={{ scale: 1.02 }}
                    className="flex items-center space-x-3 p-3 border rounded-lg hover:border-gray-300 transition-colors min-w-[180px] sm:min-w-[200px] lg:min-w-[220px] flex-grow"
                  >
                    <Checkbox
                      id={sub}
                      checked={selectedSubjects.includes(sub)}
            ...
            <div className="space-y-4">
              {selectedSubjects.map(sub => {
                if (loadingSubjects.has(sub)) {
                  return (
                    <div key={sub} className="flex items-center justify-between py-6 px-4 border rounded-lg">
                ...
                return (
                  <div key={sub} className="flex flex-wrap items-center justify-between gap-4 p-4 border rounded-lg">
                    <span className="w-[250px] capitalize text-lg font-medium">
